...
We will see a quick setup to configure the ``tool_alacritty`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_alacritty`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_alacritty`` formulas under ``/srv/salt/tool_alacritty/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_alacritty/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_alacritty/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_alacritty/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_alacritty/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_alacritty`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_alacritty`` formulas under ``/srv/salt/tool_alacritty/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_alacritty/parameters/roles

We will define 2 roles:

- ``tool_alacritty/server``
- ``tool_alacritty/client``

We create a configuration for the role ``tool_alacritty/server`` in ``/srv/salt/tool_alacritty/parameters/roles/tool_alacritty/server.yaml``:
...
We create another configuration for the role ``tool_alacritty/client`` in ``/srv/salt/tool_alacritty/parameters/roles/tool_alacritty/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_alacritty/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_alacritty/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_alacritty/parameters/dns:domain/  '/srv/salt/tool_alacritty/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-alacritty-as-json:
      test.nop:
        - name: {{ alacritty | json }}
...
.. code-block:: sls
    ...
    alacritty-config-file-file-managed:
      file.managed:
        - name: {{ alacritty.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='alacritty-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ alacritty.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            alacritty: {{ alacritty | json }}

This ``sls`` file expose a ``alacritty`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ alacritty['winner'] }}
